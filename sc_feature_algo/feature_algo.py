...
def exportresult(roiaccuracy, coughaccuracy, ypred, y_test, testindex, model, knn_n):
    #result=[]
...
plt.scatter(peaklist, y, color='red') #Plot detected peaks
x=range(0, len(motionlist) ,1)
motion = [i * 1000 for i in motionlist]
print(motion)
plt.scatter(x,motion, color='yellow')
yy = np.array(df['Cough state']) 	
plt.plot(yy, alpha=0.5, color='green') 
plt.show()
...
#print(templist)
X=featureextraction(df,templist)#obtain X, 42 columns(5 features for each sensor and 1 for motion, 1 for index)
...
X_train = X_train.iloc[:,0:41]
testindex=[]
for i in range(0,len(X_test)):
    testindex.append(X_test['Index'].iloc[i])
...
#export result
exportresult(roiaccuracy, coughaccuracy, ypred, y_test, testindex, model, knn_n)
